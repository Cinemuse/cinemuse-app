...
class _MediaDetailsScreenState extends ConsumerState<MediaDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final detailsAsync = ref.watch(
      mediaDetailsProvider((id: widget.mediaId, type: typeForTmdb)),
    );
    ...
    final bodyContent = detailsAsync.when(
      ...
      data: (details) {
        ...
        final isFavorite = ref
            .watch(userListsProvider.notifier)
            .isFavorite(tmdbId, mediaKind);
        final isInWatchlist = ref
            .watch(userListsProvider.notifier)
            .isInWatchlist(tmdbId, mediaKind);

        // Watch history data
        final watchHistory = ref
            .watch(mediaWatchHistoryProvider(widget.mediaId))
            .value;
        ...
        ref.listen(mediaWatchHistoryProvider(widget.mediaId), (previous, next) {
          ...
          if (nextHistory != null &&
              nextHistory.season != null &&
              nextHistory.season! > 0) {
        ...
        if (!_hasInitializedSeason &&
            watchHistory != null &&
            watchHistory.season != null &&
          ...
          if (seasonData != null) {
            ...
            final watchedEpisodesMap = ref.read(
              watchedEpisodesMapProvider(tmdbId),
            );
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            ref.read(selectedSeasonProvider(widget.mediaId).notifier).state =
                targetSeason;
          });
        }
        ...
        final seriesWatchStatus = isTV
            ? ref.watch(
                seriesWatchStatusProvider((
                  tmdbId: tmdbId,
                  totalEpisodes: _calculateReleasedEpisodes(details),
                )),
              )
            : null;
        final movieWatchCount = !isTV
            ? ref.watch(movieWatchCountProvider(tmdbId))
            : 0;
        final hasFinances =
              ...
              child: DetailsHero(
                ...
                resumeData: watchHistory != null
                    ? {
                        ...
                        'type':
                            (watchHistory.mediaType == MediaKind.episode ||
                                watchHistory.mediaType == MediaKind.tv)
                            ? 'tv'
                            : 'movie',
                ...
                onPlayClick: () =>
                    _handlePlay(context, watchHistory, typeForTmdb),
                onRestartClick: () => _handlePlay(
                  context,
                  watchHistory,
                  typeForTmdb,
                  restart: true,
                ...
                onDeepSearch: (params) => {},
                onListTap: () =>
                    _showAddToList(context, tmdbId, typeForTmdb, details),
                onTrackTap: isTV
                    ? () => _showSeriesTrackModal(
                        context,
                        controller,
                        tmdbId,
                        details,
                            ...
                            onShowTvTimeComments: () {
                              ...
                              if (isTV && tvdbId != null) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      l10n.detailsTvTimeCommentsNotAvailable,
                                    ),
                                  ),
                                );
                              }
  ...
  void _markAllEpisodes(
    MediaDetailsController controller,
    int tmdbId,
    Map<String, dynamic> details,
    DateTime? date, {
    required bool onlyRemaining,
  }) {
    ...
    for (var season in seasons) {
      ...
      if (lastAired != null && sNum == lastAired['season_number']) {
        epCount = (lastAired['episode_number'] as int? ?? 0);
          ...
          sNum < (lastAired['season_number'] as int? ?? 0)) {
...
class _SeriesEpisodesSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final seasonDetailsAsync = ref.watch(
      seasonDetailsProvider((tmdbId: tmdbId, seasonNumber: selectedSeason)),
    );

    final watchedEpisodesMap = ref.watch(watchedEpisodesMapProvider(tmdbId));
    final episodeProgressMap = ref
        .watch(episodeProgressMapProvider(tmdbId))
        .value;
    final isMobile = MediaQuery.of(context).size.width < 600;
          ...
          data: (season) {
            ...
            return EpisodeList(
              ...
              onShowTvTimeComments: (season, episode) {
                ...
                if (tvdbId != null) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        l10n.detailsTvTimeCommentsNotAvailableSeries,
                      ),
                    ),
                  );
                }
              ...
              onEpisodeTap: (s, e, name) {
                final progress =
                    episodeProgressMap?['$s-$e']?.progressSeconds ?? 0;
                Navigator.of(context, rootNavigator: true).push(
                  MaterialPageRoute(
                    builder: (_) => VideoPlayerScreen(
                      queryId: mediaId,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ConstrainedBox(
      ...
      child: PopupMenuButton<int>(
        ...
        onSelected: (val) =>
            ref.read(selectedSeasonProvider(mediaId).notifier).state = val,
        child: MouseRegion(
        ...
        itemBuilder: (context) => seasons
            .map(
              (s) => PopupMenuItem<int>(
                ...
                child: Text(
                  l10n.detailsSeasonNumber(s['season_number']),
