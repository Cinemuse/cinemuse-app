...
class SportTvEvent {
  ...
  String get sportTranslationKey {
    ...
    if (clean.contains('basket') || clean.contains('pallacanestro')) return 'sport_basketball';
    if (clean.contains('motociclismo') || clean.contains('motogp') || clean.contains('moto gp')) return 'sport_motorcycling';
    ...
    if (clean.contains('formula 1') || clean.contains('f1') || clean.contains('automobilismo')) return 'sport_f1';
...
class SportScheduleScraper {
  ...
  List<SportTvEvent> parseHtml(String html) {
    ...
    for (var el in entryContent.children) {
      if ((el.localName == 'h2' || el.localName == 'h3') && el.text.contains('–')) {
