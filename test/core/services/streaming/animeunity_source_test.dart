...
class MockAnimeUnityMappingProvider extends Mock
    implements AnimeUnityMappingProvider {}
  ...
  group('AnimeUnitySource', () {
    test('search should return streams when mapping and API succeed', () async {
      final context = StreamSearchContext(
        ...
        episode: 1,
        season: 1,
        mapping: KitsuMapping(kitsuId: '12', absoluteEpisode: 1),
      );

      when(() => mockMappingService.getAnimeUnityIds('12')).thenAnswer(
        (_) async => [AnimeUnityEntry(id: 999, path: '/anime/999-test')],
      );
      ...
      when(() => infoResponse.data).thenReturn({
        'episodes': [
          {'id': 1000, 'number': 1, 'file_name': 'Test.S01E01.1080p.mkv'},
        ],
      });
      ...
      when(
        () => embedUrlResponse.data,
      ).thenReturn('https://vixcloud.co/embed/1234');
